...
# Add to the black list those listings whose coordinates are beyond Paris' limits
lwn_coord = listings.loc[(listings.longitude>2.42)|(listings.longitude<=2.236)].index.values
...
blacklist = list(set(np.concatenate([lwn_name,lwn_summary, lwn_desc,
                ...
                lwn_num_zip,lwn_coord,lwn_loc_exact,
                lwn_beds_baths, lwn_rvws], axis=0)))
...
# Get the index of those listings whose zipcode_y is not null due to the merge.
